...
export class Lexer {
    ...
    private addToken(kind: SyntaxTokenKind) {
        this.tokens.push(SyntaxToken.create(kind, this.start, this.current - this.start, this.text.substring(this.start, this.current)));
    }
    ...
    gatherTrivia() {
        ...
        let lastNonTrivia: SyntaxToken | undefined = undefined;
    ...
    singleLineComment() {
        ...
        this.tokens.push(SyntaxToken.create(SyntaxTokenKind.SINGLE_LINE_COMMENT, this.start - 2, this.current - this.start + 2, this.text.substring(this.start, this.current)));
    }
    ...
    multilineComment() {
        ...
        if (this.isAtEnd()) {
            this.tokens.push(SyntaxToken.create(SyntaxTokenKind.INVALID, this.start - 2, this.current - this.start + 2, this.text.substring(this.start, this.current)));
        }
        else {
            this.tokens.push(SyntaxToken.create(SyntaxTokenKind.MULTILINE_COMMENT, this.start - 2, this.current - this.start + 4, this.text.substring(this.start, this.current)));
            this.skip(2);
        }
    }
